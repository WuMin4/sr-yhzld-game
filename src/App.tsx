              ...
              <div className="card-panel p-6 rounded-xl border-dashed border-[var(--color-neon-blue)] flex flex-col items-center justify-center min-h-[200px]">
                ...
                {gameState.phase === 'GAME_OVER' && (
                  <div className="flex flex-col items-center gap-6 mt-4">
                    <div className="text-[var(--color-neon-gold)] text-3xl font-bold font-display animate-bounce">
                      游戏结束！
                    </div>
                    {isHost ? (
                      <button 
                        className="glow-btn bg-[var(--color-neon-pink)] text-black px-8 py-3 rounded-lg font-bold text-lg cursor-pointer"
                        onClick={() => dispatch({ type: 'RESTART_GAME' })}
                      >
                        再来一局
                      </button>
                    ) : (
                      <div className="text-gray-400 animate-pulse">等待房主重新开始...</div>
                    )}
                  </div>
                )}
              </div>
