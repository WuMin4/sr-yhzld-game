...
const getPairs = (counts: Record<number, number>) => {
  return Object.values(counts).reduce((acc, count) => acc + Math.floor(count / 2), 0);
};
...
const resolveCombat = (state: GameState) => {
  ...
  if (atkHasHack && defDice.length > 0) {
    const maxDefIndex = defValues.indexOf(Math.max(...defValues));
    defValues[maxDefIndex] = 1;
    defDice[maxDefIndex].value = 1;
    appendLog(state, `${attacker.name} 触发了【骇入】，将防守方最大骰子变为了1`, 'effect');
  }
  if (defHasHack && atkDice.length > 0) {
    const maxAtkIndex = atkValues.indexOf(Math.max(...atkValues));
    atkValues[maxAtkIndex] = 1;
    atkDice[maxAtkIndex].value = 1;
    appendLog(state, `${defender.name} 触发了【骇入】，将攻击方最大骰子变为了1`, 'effect');
  }
  ...
  if (attacker.statuses.strength > 0) {
    ...
    appendLog(state, `${attacker.name} 消耗了所有【力量】，攻击值增加 ${attacker.statuses.strength}`);
    attacker.statuses.strength = 0;
  }
  ...
  if (attacker.characterId === 'fengjin') {
    if (atkValues.length > 0 && atkValues.every(v => v === 6)) {
      ...
      appendLog(state, `${attacker.name} 触发风堇效果，记录力量 ${atkValue} 并回复3点生命`, 'skill');
    } else {
...
export const processAction = (prevState: GameState, action: GameAction): GameState => {
  ...
  switch (action.type) {
    case 'JOIN_GAME': {
      ...
      // Select 2 random chars for each player
      state.phase = 'CHAR_SELECT';
      const availableChars = [...CHARACTERS].sort(() => Math.random() - 0.5);
      state.charSelectChoices[state.hostId] = availableChars.splice(0, 2).map(c => c.id);
      state.charSelectChoices[state.guestId] = availableChars.splice(0, 2).map(c => c.id);
    ...
    case 'RESTART_GAME': {
      ...
      appendLog(state, `========== 游戏重新开始 ==========`, 'info');
      appendLog(state, `请选择你们的角色。`, 'info');
      break;
    }
  }
